...
//
// Usage: node scripts/build-density.mjs [--force]
...
async function main() {
  ...
  try {
    ...
    console.warn(`Could not load population raster (${err.message}).`);
    console.warn("Falling back to a coarse synthetic grid from world-atlas geometry.");
    return writeGrid(syntheticGrid(), { resolution: "synthetic", source: "synthetic" });
  }
...
function aggregate(csv) {
  const bins = new Map(); // key -> { pop, byIso: Map<iso3, pop> }
  let line = 0;
