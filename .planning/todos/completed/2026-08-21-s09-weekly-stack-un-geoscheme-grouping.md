...
decisions:
  ...
  - Rejected a 10% floor here, then adopted it on 2026-08-31 in c3e9c037. The rejection's own numbers held up on remeasurement — it predicted Elsewhere at 15.6% of the window and 22% in its worst week, and the measured figures are 15.1% and 20.4% — so the reversal is a taste call taken with the trade-off in view, not a measurement corrected. What this entry got wrong was treating "names the same five countries" as the whole of the change; it also decides how far the rollup climbs, and at 10% it reaches whole continents.
...
superseded: 2026-08-31 — the floor is 10%, not the 5% this file shipped (c3e9c037). Elsewhere goes from 6.4% of the window to 15.1% and from 7.8% to 20.4% in its worst week; bands per bar fall from 10-12 to 5-7; countries ever named fall from 12 to 5, with Pakistan, Brazil, Ethiopia, Sudan, Lebanon, Somalia and Yemen losing their bands; and Asia and Africa now stand as bands beside Latin America and the Caribbean. Nesting improved rather than worsened — because fewer groups stop at subregion level while a sibling stands at continent level, the odd "Western Africa beside Africa" pairing fell from all twelve bars to four. Everything this file built is otherwise untouched — the cascade, per-week membership, the residual absorb step and colour keyed to place all behave the same against a higher bar, which is the evidence that one constant was the right shape for this knob. Note for any future change to it — a derived field in data/conflicts.json can be migrated in place from the stored payload, because each band carries its members with their own fatalities and so reconstructs buildWeeklyStack's input map exactly. Only a change to what ACLED itself gives us needs a refetch.
