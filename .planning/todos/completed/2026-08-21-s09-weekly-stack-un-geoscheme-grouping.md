...
area: data
resolved: >-
  2026-08-30 — shipped in 3a4b98ba. Floor shipped at 5%, raised to 10% on 2026-08-31 (see superseded
  below), and it governs both halves: which countries are named, and how far a leftover coarsens
  before it can stand. Sub-floor countries group by M49 subregion, climb to the intermediary region
  where one exists (419 LAC, 202 Sub-Saharan Africa), then to the continent, stopping at the first
  level clearing 5% of that week. Elsewhere fell from 51.6% of the window to 6.4%, and every band
  drawn is at least 5% of its own bar. Membership is per week, not global — 2-6 countries clear 5%
  in any given week, so a global list would name a country in every bar on the strength of one bad
  week. That forced the payload shape (segments[] with a kind and its members, schema 2 -> 3) and
  colour keyed to place rather than slot. data/conflicts.json was migrated in place, not refetched:
  the v2 stack reconstructs its input map exactly.
decisions:
  - >-
  ...
  - >-
    ...
    reads as one band. Greedy over the "ever drawn in the same bar" graph needs exactly the 6 and
    the 8 available.
  - >-
    Rejected a 10% floor here, then adopted it on 2026-08-31 in c3e9c037. The rejection's own
    numbers held up on remeasurement — it predicted Elsewhere at 15.6% of the window and 22% in
    its worst week, and the measured figures are 15.1% and 20.4% — so the reversal is a taste call
    taken with the trade-off in view, not a measurement corrected. What this entry got wrong was
    treating "names the same five countries" as the whole of the change; it also decides how far
    the rollup climbs, and at 10% it reaches whole continents.
  - >-
    Countries with no M49 (ACLED's "Pacific Ocean" and friends) never get their own band. One
    cleared 5% in a week and would have spent a band and a colour on a non-place.
  - >-
    Region names are translated, unlike country and cause names: these are labels we mint, not
    wording arriving from the data files.
superseded: >-
  2026-08-31 — the floor is 10%, not the 5% this file shipped (c3e9c037). Elsewhere goes from 6.4%
  of the window to 15.1% and from 7.8% to 20.4% in its worst week; bands per bar fall from 10-12 to
  5-7; countries ever named fall from 12 to 5, with Pakistan, Brazil, Ethiopia, Sudan, Lebanon,
  Somalia and Yemen losing their bands; and Asia and Africa now stand as bands beside Latin America
  and the Caribbean. Nesting improved rather than worsened — because fewer groups stop at subregion
  level while a sibling stands at continent level, the odd "Western Africa beside Africa" pairing
  fell from all twelve bars to four. Everything this file built is otherwise untouched — the
  cascade, per-week membership, the residual absorb step and colour keyed to place all behave the
  same against a higher bar, which is the evidence that one constant was the right shape for this
  knob. Note for any future change to it — a derived field in data/conflicts.json can be migrated in
  place from the stored payload, because each band carries its members with their own fatalities and
  so reconstructs buildWeeklyStack's input map exactly. Only a change to what ACLED itself gives us
  needs a refetch.
test_finding: >-
  The M49 coverage test — every ISO code i18n-iso-countries knows must resolve to a chain ending at
  a continent — caught two holes on its first run. Burkina Faso was missing from Western Africa
  outright and was silently losing 432 deaths in one week to the residual, which looks like working
  software. Kosovo has no M49 code at all; its user-assigned 983 is mapped to Southern Europe with a
  comment. Any hand-authored code table wants this test.
files:
