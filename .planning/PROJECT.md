...
| Read causes at 2019, not at the newest reference year the release offers | At 2021, covid-19 was ~30% of mid-life cause weight in India and Brazil and 0% in Japan and China, drawn by a present-tense feed as how people ordinarily die. It is also the rule the seasonality pipeline already followed and the cause table alone did not (`COVID_YEARS`) | ✓ Good — but see the vintage spread below |
| Name the release vintage and the reference year separately   | They differ — this is the 2021 release read at its 2019 back-series — and "GHE 2019" names a real, different release built with superseded methods; a single number would have been a false citation | ✓ Good — both are in the prose and both are asserted by `storyTranslations.test.ts` |
| Accept a model composed of six vintages spanning 2015-2024   | Each layer takes the best year its source offers for what it is asked. Density 2015 is a shape the 2024 rate is scaled onto by construction, so that gap is exact arithmetic; the 2019 cause mix against a 2024 count is the one assumption with nothing reconciling it | ⚠️ Revisit — defensible but only two of the six vintages are stated anywhere the reader sees |
