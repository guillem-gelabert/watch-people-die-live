...
interface CreateEarthParams {
  dayTexture: Texture;
  nightTexture: Texture;
  bumpRoughnessCloudsTexture: Texture;
  sunDirection: Vector3; // unit, in the texture/world frame
  atmosphereDayColor: string; // hex string
  atmosphereTwilightColor: string; // hex string
  nBlasts: number;
  blastMaxR: number;
  blastWidth: number;
  blastAmp: number;
...
export interface EarthMaterials {
  globeMaterial: MeshStandardNodeMaterial;
  atmosphereMaterial: MeshBasicNodeMaterial;
  sunDir: UniformNode<"vec3", Vector3>;
  blastUv: ReturnType<typeof uniformArray<"vec2">>;
  blastProg: ReturnType<typeof uniformArray<"float">>;
  blastCount: UniformNode<"float", number>;
}
...
export function createEarth({
  ...
  blastWidth,
  blastAmp,
}: CreateEarthParams): EarthMaterials {
  // Uniforms driven from the render loop each frame / minute.
  ...
  const blastUv = uniformArray<"vec2">(
    Array.from({ length: nBlasts }, () => new Vector2()),
    "vec2",
  );
  const blastProg = uniformArray<"float">(new Array(nBlasts).fill(0), "float");
  ...
  const roughness = max(texture(bumpRoughnessCloudsTexture, ripUv).g, step(0.01, cloudsStrength));
