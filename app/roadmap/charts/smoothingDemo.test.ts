...
import { SMOOTHING_MODES, selectSmoothingSeries, smoothingMode } from "./smoothingDemo";
...
describe("smoothing explainer configuration", () => {
  it("has the five stable mode keys and complete explanatory fields", () => {
    expect(SMOOTHING_MODES.map((mode) => mode.key)).toEqual([
      "weekly",
      "monthly",
      "quarterly",
      "circular3",
  ...
  it("updates the harmonic explanation with the selected order", () => {
    expect(smoothingMode("circular3").label).toBe("Circular 3-point");
    expect(smoothingMode("harmonic", 3).label).toBe("Harmonic · order 3");
    expect(smoothingMode("harmonic", 4).goodFor).toContain("production model");
  });
});
